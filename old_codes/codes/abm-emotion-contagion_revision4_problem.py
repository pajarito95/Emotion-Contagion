...
##############
folderpath = r"C:\Users\SEscottoRo\OneDrive - Toyota Industries Group North America\Documents\My stuff\other\Masters\523\Simulation\Leader L\Results=same charisma"  # in the quotations, put the name of the folder in which you want the results to be saved to  
def save_results(avg_emotion_runs, folderpath, images=None):
    ...
    '''
    # Determine the next results folder number
    existing = [
        name for name in os.listdir(folderpath)
        if os.path.isdir(os.path.join(folderpath, name)) and name.startswith("Results")
    ]

    run_nums = []
    for name in existing:
        try:
            run_num = int(name.split("Results")[-1].strip())
            run_nums.append(run_num)
        except ValueError:
            continue

    next_run = max(run_nums, default=-1) + 1
    subfolder_name = f"Results {next_run}"
...
def initialize(seed=None):
    ...
    Agent parameters:
        Emotion (D): How an agent feels towards the change. Defined within the interval [-1, 1] following a beta distribution (2, 5) which skews towards the negative side.
                     The purpose behind the interval is because we want to see how a leader's emotional aperature dictates the evolution of the emotion contagion which gives more meaningful results when emotions initially tend more negative. This will also encourage the random number picker towards negative.
    ...
    emotionManagementAbility = 'L'  # we select whether H or L, just replace as desired -- ALSO CHANGE IN LINE 326
...
def run_simulation(seed=0, run_id=0, save_folder=None):
    ...
    images = [(fig, f"Sentiment_Evolution_Run{run_id}_Seed{seed}.png")]  # list of images to save
    ...
    # Package up the results  -- we should add in code to save the results (and we also need to have the graphs saved as well) so we have access to them
    results = {
        'leader': leader_data,
        ...
        'fig': fig,  # save the figure object for later use
    }
...
def run_multiple_simulations(runs=30, save_folder=folderpath):
    ...
    for run in range(runs):
        ...
        agents = []
        leader = {'Emotion': 1, 'emotionManagementAbility': 'L', 'interventionThreshold': -0.2}
        emotionHistory = []
        avgEmotionHistory = []
        ...
        fig = results['fig']  # get the figure object from the results
        filename = f"Sentiment_Evolution_Run{run+1}_Seed{run}.png"
        images.append((fig, filename))  # add the figure and filename to the images list
        
    if save_folder:
        save_results(avgEmotionHistory, save_folder, images)
...
runs = 30
results = run_multiple_simulations(runs=runs)
max_len = max(len(run) for run in results)

# mean_emotions = np.mean(results, axis=0)
# std_emotions = np.std(results, axis=0)

# # Plot with error bands
# timesteps = range(len(mean_emotions))
# fig, ax = plt.subplots()
# ax.plot(timesteps, mean_emotions, label="Mean Avg Emotion")
# ax.fill_between(timesteps,
#                  mean_emotions - std_emotions,
#                  mean_emotions + std_emotions,
#                  alpha=0.3, label="±1 Std Dev")
# ax.set_xlabel("Time")
# ax.set_ylabel("Avg Emotion")
# ax.set_title(f"Emotion Evolution over {runs} Runs")
# ax.set_ylim(-1, 1)  # set y-axis limits to be between -1 and 1
# ax.legend()
# ax.grid(True)
# plt.show()
# fig.tight_layout()
# images = [(fig, "Avg_Emotion_Evolution_Over_Runs.png")]  # list of images to save

save_results(results,
             folderpath=folderpath, 
             images=None) #[(fig, "AvgEmotionPlot.png")])


folderpath = r"C:\Users\SEscottoRo\OneDrive - Toyota Industries Group North America\Documents\My stuff\other\Masters\523\Simulation\Leader L\Intervention"
#os.makedirs(folderpath, exist_ok=True)

# Save interventions list
intervention_lists = results[1]  # from run_multiple_simulations()
df_interventions = pd.DataFrame({
    "Run": list(range(1, runs + 1)),
    "Intervention_Timesteps": [','.join(map(str, run)) for run in intervention_lists],
    "Num_Interventions": [len(run) for run in intervention_lists]
})

# df_interventions.to_csv(os.path.join(folderpath, "Intervention_Timesteps_Summary_Low.csv"), index=False)

# save above as boolean matrix
max_time = max((max(run) if run else 0) for run in intervention_lists) + 1  # ensure room for last timestep
binary_matrix = np.zeros((runs, max_time), dtype=int)
for i, run in enumerate(intervention_lists):
    for t in run:
        binary_matrix[i, t] = 1

df_bool = pd.DataFrame(binary_matrix).T
