...
def update_intimacy_matrix(
    ...
    if intimacy.ndim != 2 or intimacy.shape[0] != intimacy.shape[1]:
        raise ValueError(
"update_intimacy_matrix expected a square 2D intimacy matrix."
)
    if intimacy.shape[0] != n_agents:
        raise ValueError(
f"Intimacy matrix size ({intimacy.shape[0]}) does not match number "
f"of agents ({n_agents})."
)
    if not (0 <= decay <= 1):
    ...
    member_block = member_block / (row_sums + eps)
    A[np.ix_(member_indices, member_indices)] = member_block
...
def emotional_valence_update(
    ...
    """
    Update the emotional valence of two interacting member agents according to the Bosse-style contagion rule.
    Only member-member interactions are valid here.
    ...
    gamma_A = sum(
        sender["expressiveness"]
        * intimacyMatrix[sender["index"], agentA["index"]]
        * agentA["delta"]
        for sender in member_agents if sender is not agentA
    )
    gamma_B = sum(
        sender["expressiveness"]
        * intimacyMatrix[sender["index"], agentB["index"]]
        * agentB["delta"]
        for sender in member_agents if sender is not agentB
    )
    ...
    groupEmos_A = sum(
        other["expressiveness"] * intimacyMatrix[other["index"], agentA["index"]]
        for other in member_agents if other is not agentA
    )
    groupEmos_B = sum(
        other["expressiveness"] * intimacyMatrix[other["index"], agentB["index"]]
        for other in member_agents if other is not agentB
    )
    ...
    agentA["emotion"] += gamma_A * (
        eta_A 
        * (beta_A * PI_A + (1 - beta_A) * NI_A) 
        + (1 - eta_A) * qstar_A 
        - initial_qA
        )
    ...
    agentB["emotion"] += gamma_B * (
        eta_B 
        * (beta_B * PI_B + (1 - beta_B) * NI_B)
        + (1 - eta_B) * qstar_B
        - initial_qB
        )
