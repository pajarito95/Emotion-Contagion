...
def _validate_seeds(seeds: Sequence[int]) -> None:
    ...
    for seed in seeds:
        if not isinstance(seed, int):
            raise TypeError(
                f"Each seed must be an integer, but received {type(seed).__name__}."
            )
...
def _validate_conditions(conditions: Sequence[Dict[str, Any]]) -> None:
    ...
    for i, condition in enumerate(conditions):
        ...
        if not isinstance(condition, dict):
            raise TypeError(
                f"conditions[{i}] must be a dictionary, "
                f"but received {type(condition).__name__}."
            )
        ...
        if missing:
            raise KeyError(
                f"conditions[{i}] is missing required key(s): {sorted(missing)}."
            )
...
def _build_single_condition_from_arguments(
    ...
    if missing:
        raise ValueError(
            f"When conditions is not provided, these are required: {missing}"
        )
...
def _results_to_summary_row(
    ...
    return {
        ...
        "final_min_member_emotion":
            min(final_member_emotions)
            if final_member_emotions else None,

        "final_max_member_emotion":
            max(final_member_emotions)
            if final_member_emotions else None,

        "mean_rl_reward":
            float(np.mean(results.rl_rewards))
            if results.rl_rewards else None,

        "mean_rl_quality":
            float(np.mean(results.rl_quality))
            if results.rl_quality else None,
    }
...
def write_batch_note(
    ...
    if conditions_to_run:
        ...
        leader_styles = sorted({
            c.get("leader_style")
            for c in conditions_to_run
        })

        max_iterations = sorted({
            c.get("max_iterations")
            for c in conditions_to_run
        })
...
def run_multiple_simulations(
    ...
    metadata_filename_template:
        str = "simulation_result_run_{run_id}_metadata.json",

    qtable_filename_template:
        str = "simulation_result_run_{run_id}_qtable.pkl",
    ...
    if provided_modes == 0:
        raise ValueError(
            "Provide conditions, condition_grid, "
            "or direct single-condition arguments."
        )

    if conditions is not None and condition_grid is not None:
        raise ValueError(
            "Provide either conditions or condition_grid, not both."
        )
    ...
    for condition_index, condition in enumerate(
        ...
        current_condition_name = condition.get(
            "condition_name",
        ...
        for seed in seeds:
            ...
            results = run_simulation(
                ...
                include_leader_ties=condition.get(
                    "include_leader_ties",
                    True,
                ),

                leader_to_member_cap=condition.get(
                    "leader_to_member_cap"
                ),

                member_to_leader_cap=condition.get(
                    "member_to_leader_cap"
                ),

                leader_to_member_value=condition.get(
                    "leader_to_member_value"
                ),

                member_to_leader_value=condition.get(
                    "member_to_leader_value"
                ),
                ...
                adaptive_intimacy=condition.get(
                    "adaptive_intimacy",
                    False,
                ),
                ...
                rl_epsilon_start=condition.get(
                    "rl_epsilon_start",
                    0.3,
                ),

                rl_epsilon_end=condition.get(
                    "rl_epsilon_end",
                    0.05,
                ),

                rl_epsilon_decay_steps=condition.get(
                    "rl_epsilon_decay_steps",
                    2000,
                ),

                rl_actions=condition.get(
                    "rl_actions",
                    [0, 1, 2, 3],
                ),
            ...
            if save_full_results or save_run_metadata or save_q_tables:
                ...
                if save_full_results:
                    ...
                    results_path = (
                        run_output_dir
                        / results_filename_template.format(run_id=run_id)
                    )
                ...
                if save_run_metadata:
                    ...
                    metadata_path = (
                        run_output_dir
                        / metadata_filename_template.format(run_id=run_id)
                    )
                ...
                if (
                    save_q_tables
                    and results.q_table is not None
                ):

                    qtable_path = (
                        run_output_dir
                        / qtable_filename_template.format(run_id=run_id)
                    )
