...
- The leader is identified by `leader_index` and is expected to be the last agent in the list.
- One intimacy matrix storing all member ties.
...
def run_simulation(
    ...
    initial_homophily = compute_homophily_index( agents=state.agents, intimacy_matrix=state.intimacy_matrix)
    ...
    while time < max_iterations:
        ...
        # HOMOPHILY
        homophily_t = compute_homophily_index(agents=state.agents, intimacy_matrix=state.intimacy_matrix)
