...
def make_timestamp_string() -> str:
    """
    Create a timestamp string safe for folder names.
    """
...
def _validate_seeds(seeds: Sequence[int]) -> None:
    """
    Validate the sequence of seeds.
    """
...
def _validate_conditions(conditions: Sequence[Dict[str, Any]]) -> None:
    """
    Validate the sequence of condition dictionaries.
    """
...
def _make_run_output_dir(
    ...
    """
    Create output folders for condition-level and/or run-level organization.
    """
...
def save_run_metadata_json(metadata: Dict[str, Any], filepath: Path) -> None:
    """
    Save readable run metadata as JSON.
    """
...
def build_conditions_from_grid(
    ...
    """
    Build a list of condition dictionaries from a parameter grid using a Cartesian product.

    Parameters
    ----------
    grid : dict[str, sequence]
        Parameters to vary. Each key maps to a sequence of possible values
    fixed_params : dict or None, optional
        Parameters that should be included unchanged in every condition
    condition_name_keys : sequence[str] or None, optional
        Keys to use when auto-building the condition_name string. If None, all grid keys are used
...
def _build_single_condition_from_arguments(
    ...
    community_size: Optional[tuple[int, int]],
    min_weight: float,
    core_to_core: float,
    ...
    """
    Build one condition dictionary from direct function arguments.
    """
    required = {
...
def _build_run_metadata(
    ...
    """
    Build readable metadata for a single saved run.
    """
    return {
        "run_id": results.run_id,
        "seed": results.seed,
...
def run_multiple_simulations(
    ...
    Supported modes:
    1. Single-condition mode via direct arguments.
    2. Multi-condition mode via conditions=[...].
    3. Grid mode via condition_grid={...} plus optional fixed_params={...}.

    Returns
    -------
    dict
        Dictionary containing results, summary DataFrame, output paths, and conditions used
    """
    ...
    for condition_index, condition in enumerate(conditions_to_run, start=1):
        ...
        for seed in seeds:
            ...
            results = run_simulation(
                ...
                intra_strength=condition.get("intra_strength", 0.5),
                inter_strength=condition.get("inter_strength", 0.2),
                community_size=condition.get("community_size"),
                min_weight=condition.get("min_weight", 0.01),
                core_to_core=condition.get("core_to_core", 0.65),
                core_to_periph=condition.get("core_to_periph", 0.5),
                periph_to_core=condition.get("periph_to_core", 0.2),
                periph_to_periph=condition.get("periph_to_periph", 0.2),
                core_proportion=condition.get("core_proportion", 0.25),
