...
def social_network_graph(buddies):
    '''
    ...
    Idea 2 (directed): two edges maximum allowed between agents (one in each direction) representing the exchange of information from one agent to the other
        Now, how is this information exchange represented? The absolute value of the difference in old emotion and new emotion will get appended to the edge weight
    '''
    ...
    G.add_edges_from(buddies)  # add edges between agents based on the buddies list
    # now, how to add weights...
...
def run_simulation(seed=0, run_id=0, save_folder=None):
    ...
    print(f'Interactions per timestep: {interactions_per_timestep}')
