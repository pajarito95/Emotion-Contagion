...
def initialize():
    ...
    for agent in range(populationSize):  # for each agent in the population
        newAgent = {'Emotion': uniform(-0.3,1), 'alpha': uniform(0,1)}  # dictionary for parameter readability
    ...
    leader['alpha'] = 0  # set their alpha to 0 -- we could also just delete it
    ...
    ### [CURRENTLY REDUNDANT] calculate expressiveness -- since we normalize the intimacyMatrix above, it rendered this redundant since they all will sum to 1 now
...
def emotional_valence_update(agent_r, agent_c, agent_r_index, agent_c_index):
    ...
    '''
    gamma_r = 0.5  # the gamma value for the agent_r
    eta_r = 0.5  # the eta value for the agent_r
    beta_r = 0.5  # the beta value for the agent_r
    q_r = agent_r['Emotion']
    q_star = [sum(agent['Emotion']) for agent in agents if agent != agent_r]
    ...
    #if agent_c['Emotion'] > agent_r['Emotion']:
        #agent_r['Emotion'] += agent_r['alpha']*((agent_c['Emotion'] - agent_r['Emotion'])/(agent_c['Emotion'] - agent_r['Emotion'])**2+0.0001)
        #agent_c['Emotion'] += agent_c['alpha']*((agent_c['Emotion'] - agent_r['Emotion'])/(agent_c['Emotion'] - agent_r['Emotion'])**2+0.0001)
        
        # agent_r['Emotion'] += agent_r['alpha']*(1/(agent_c['Emotion'] - agent_r['Emotion']))
        # agent_c['Emotion'] += agent_c['alpha']*(1/(agent_c['Emotion'] - agent_r['Emotion']))

    #elif agent_c['Emotion'] < agent_r['Emotion']:
        #agent_r['Emotion'] += agent_r['alpha']*((agent_r['Emotion'] - agent_c['Emotion'])/(agent_r['Emotion'] - agent_c['Emotion'])**2+0.0001)
        #agent_c['Emotion'] += agent_c['alpha']*((agent_r['Emotion'] - agent_c['Emotion'])/(agent_r['Emotion'] - agent_c['Emotion'])**2+0.0001)

        # agent_r['Emotion'] += agent_r['alpha']*(1/(agent_r['Emotion'] - agent_c['Emotion']))
        # agent_c['Emotion'] += agent_c['alpha']*(1/(agent_r['Emotion'] - agent_c['Emotion']))
    #else:  # when both are equal
        #print("Equal emotions")
        #agent_r['Emotion'] += agent_r['alpha']*agent_r['Emotion']*(1- agent_c['Emotion'])  # just the interaction
        #agent_c['Emotion'] += agent_c['alpha']*agent_c['Emotion']*(1- agent_r['Emotion'])  # so hungry, so tired
...
def leader_intervention():
    ...
    if avgEmotionValence <= leader['interventionThreshold']:  
        for agent in agents:
            agent['Emotion'] += agent['alpha']*(1/(leader['Emotion'] - agent['Emotion']))  # multiplying by an agents susceptibility makes the intervention impact vary per agent
...
def run_simulation():
    ...
    while not convergence and time < max_iterations:
        agent_interaction()
