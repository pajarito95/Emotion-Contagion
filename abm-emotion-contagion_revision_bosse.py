...
def initialize(seed=None):
    ...
    for _ in range(populationSize):  # for each agent in the population
        newAgent = {'Emotion': -1 + 2 * np.random.beta(2, 5), 
                    ...
                    'amplificationBias': random.uniform(0,1),
                    'index': }  # dictionary for parameter readability
...
def emotional_valence_update(agentA, agentB, agentA_index, agentB_index):  # are the matrix and dictionary indices aligning?
    ...
    # gamma
    # calculate the contagion strength (influence strength) of agentA on agentB and vice versa
    # AB_contagionStrength = agentB['Expressiveness']*BA_intimacy*agentA['alpha']
    # BA_contagionStrength = agentA['Expressiveness']*AB_intimacy*agentB['alpha']

    contagionStrengthA = sum(agent in [ag for ag in agents if ag != agentA], agent['Expressiveness']*intimacyMatrix[agent['index'],agentA['index']]*agentA['alpha'])


    # q_star: weighted sum of emotions based on expressiveness and intimacy
    q_star = 0
    for agent in [a for a in agents if a != agentA]:
        for agentC in [c for c in agents if c != agentA]:
            weightedEmos = (agent['Expressiveness'] * intimacyMatrix[agent['index'], agentA['index']]) / (agentC['Expressiveness'] * intimacyMatrix[agentC['index'], agentA['index']]) * agent['Emotion']
            q_star += weightedEmos

    positiveReinforcement = 1 - (1 - q_star)*(1 - agentA['Emotion'])
    negativeReinforcement = q_star*agentA['Emotion']

    agentA['Emotion'] = contagionStrengthA*(
        agentA['Amplification']*(
            agentA['amplificationBias']*(positiveReinforcement) + (1 - agentA['amplificationBias'])*negativeReinforcement
        ) + (1 - agentA['Amplification'])*q_star - agentA['Emotion']
    )

    # Former emotion update formula
